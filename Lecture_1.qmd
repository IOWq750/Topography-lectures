---
title: "Картография с основами топографии"
subtitle: "Лекция 1. Системы координат"
author: "Карпачевский А.М."

format:
  revealjs:
    css: styles.css
    theme: simple
    transition: fade

    # Размер под проекторы и записи экрана
    width: 1280
    height: 720
    margin: 0.06
    minScale: 0.7
    maxScale: 1.0

    # Навигация
    slide-number: true
    hash: true
    navigation-mode: linear

    # Колонтитул
    footer: |
      {{< meta author >}} · {{< meta title >}} · {{< meta subtitle >}}

    # Поведение
    center: false
    progress: true
    controls: true

    # Для iframe (GeoGebra)
    embed-resources: false
---


## Общая информация по курсу

<div style="display:grid; grid-template-columns: 1fr 360px; gap: 40px; align-items:start;">

<div>

- Лекции раз в две недели (ГЗ 02)
- Практические занятия каждую неделю (см. расписание)
- Кабинет топографии (1913)
- Электронная ведомость: → → → → → → → 
- Промежуточная аттестация — практические работы + устный зачёт
- Консультации по самостоятельному заданию в QGIS: с марта (1919)

</div>

<div style="text-align:center;">
<img src="Lecture_1_files/pics/QR_Table.png" style="width:340px;">
</div>

</div>


---

## Литература

<div class="book-row">


<img src="Lecture_1_files/pics/Bozhok.jpg">
<img src="Lecture_1_files/pics/Berlyant.png">
<img src="Lecture_1_files/pics/Knizhnikov.jpg">
<img src="Lecture_1_files/pics/Lurie.jpg">

</div>

<div class="refs-small">
- **Топография с основами геодезии**: учебник для студ. географ. спец. ун-тов / А. П. Божок и др.; под ред. А. С. Харченко и А. П. Божок. — М.: Высш. шк., 1986. — 304 с.
- **Курошев Г. Д.** Топография: учебник для студ. учреждений высш. проф. образования. — М.: Академия, 2011. — 192 с.
- **Берлянт А. М.** Картография: учебник для вузов. — 2-е изд., испр. и доп. — М.: КДУ, 2011. — 464 с.
- **Аэрокосмические методы географических исследований** / Ю. Ф. Книжников и др. — 2-е изд. — М.: Академия, 2011. — 416 с.
- **Лурье И. К.** Геоинформационное картографирование. — М.: КДУ, 2010. — 424 с.

</div>

---

## Общие понятия. Системы координат в картографии

 

---

## Предметная область

- **Геодезия**  – наука о методах определения фигуры и размеров Земли и изображении её поверхности на картах, а также о разработке способов, приёмов и средств геодезических измерений на земной поверхности
- **Топография**  – научно-техническая дисциплина, занимающаяся географическим и геометрическим изучением местности путём создания топографических карт на основе съёмочных работ
- **Картография**  – наука о картах как об особом способе изображения действительности, об их создании и использовании
- **Геоинформатика**  – научно-техническое направление, объединяющее теорию цифрового моделирования предметной области с использованием пространственных данных, технологии создания и использования геоинформационных систем, производство геоинформационной продукции и оказанием геоинформационных услуг

---

## Форма и размеры Земли

<img src="Lecture_1_files/pics/Turtle.jpg">

---

## Форма Земли

<div style="display:grid; grid-template-columns: 1.2fr 1fr; gap: 40px; align-items:start;">

<div>

В большинстве случаев Земля аппроксимируется  
формой эллипсоида вращения (сфероид)

- *a*, *b* — большая и малая полуось
- $$\alpha = \frac{a - b}{a}$$ — полярное сжатие

Существует два типа эллипсоидов:
<div class="compact-list">
- **Общеземные**
- **Референц-эллипсоиды**
</div>
</div>

<div style="text-align:center;">

<img src="Lecture_1_files/pics/Spheroid.png"
     style="width:100%; max-width:420px;">

</div>

</div>


---

## Важнейшие земные эллипсоиды

<div class="ellipsoid-grid">

<div class="ellipsoid-table">

| **Автор** | **Год** | **Большая полуось, м** | **Сжатие** |
|---|---:|---:|---:|
| Деламбр | 1800 | 6 375 653 | 1/334 |
| Вальбек | 1819 | 6 376 896 | 1/303 |
| Бессель | 1841 | 6 377 397 | 1/299 |
| Кларк | 1866 | 6 378 206 | 1/295 |
| Хейфорд | 1909 | 6 378 388 | 1/297 |
| Красовский | 1940 | 6 378 245 | 1/298,3 |
| GRS-67 | 1967 | 6 378 160 | 1/298 |
| GRS-80 | 1979 | 6 378 137 | 1/298 |

</div>

<div class="ellipsoid-table">

|  | **WGS-84** | **ПЗ-90.11** | **Красовского** | **ГСК-2011** |
|---|---:|---:|---:|---:|
| *a* | 6 378 137 | 6 378 136 | 6 378 245 | 6 378 136,5 |
| *b* | 6 356 752,314 | 6 356 751,362 | 6 356 863,019 | 6 356 751,758 |
| *α* | 1/298,257 2235 | 1/298,257 84 | 1/298,3 | 1/298,256 4151 |

</div>

</div>

<div class="ellipsoid-refs">
Параметры Земли 1990-го года — справочник: <https://structure.mil.ru/files/pz-90.pdf>  
Параметры ГСК-2011: <https://docs.cntd.ru/document/420347207>
</div>


---

## Системы координат, используемые в картографии

- Геодезические (географические)
- Высотные
- Плоские прямоугольные
- Пространственные прямоугольные
- Полярные

 

---

## Геодезическая (географическая) система координат

- **Датум  (геодезический  датум , исходные даты)** – набор параметров, описывающих положение эллипсоида внутри тела Земли
- Во многих случаях  датум  – это синоним геодезической системы координат
- Разные  датумы  могут использовать одни и те же эллипсоиды
- Параметры  датума:
  - Сдвиг центра эллипсоида относительно центра масс Земли
  - Ориентация и поворот осей
  - Масштаб эллипсоида
  - Временная эпоха
 

---

## Геодезические (географические) координаты

:::: {.columns}

::: {.column width="40%"}

- **Широта  $\varphi$** — угол между плоскостью экватора и отвесной линией, проведённой в точке на поверхности Земли
- **Долгота $\lambda$** — двугранный угол между плоскостью меридиана, проходящего через данную точку, и плоскостью нулевого меридиана

:::

::: {.column width="60%"}

<iframe
  title="Lat Lon"
  src="https://www.geogebra.org/material/iframe/id/vu4xs5gp/sfsb/true/smb/false/stb/false/stbh/false/ai/false/asb/false/sri/false/rc/false/ld/false/sdz/false/ctl/false"
  style="width:100%; height:70vh; border:0; overflow:hidden;"
  allowfullscreen>
</iframe>

:::

::::

---

## Системы счёта высот

![](Lecture_1_files/pics/Heights.png){style="height:50vh; object-fit:contain;"}

**Высоты** — расстояния от поверхности Земли до определённой поверхности отсчёта, измеренные по отвесной линии

- **Геодезические высоты** — от эллипсоида
- **Ортометрические высоты** — от геоида
- **Нормальные высоты** — от квазигеоида

---

## Геоид

**Геоид** — эквипотенциальная (уровенная) поверхность, совпадающая со средней
поверхностью морей и океанов, не возмущённых приливами и отливами,
ветрами и изменениями атмосферного давления, и продолженная под
материками всюду нормально отвесным линиям [Листинг, 1873]

<div style="display:grid; grid-template-columns: 1fr 1fr; gap: 36px; margin-top:0.6em; align-items:start;">

<!-- ЛЕВО: фото с подписью поверх -->
<div class="img-with-caption">

<img src="Lecture_1_files/pics/Kronshtadt.jpg">

<div class="img-caption">
_Кронштадтский футшток_
</div>

</div>

<!-- ПРАВО: гифка геоида -->
<div style="text-align:center;">
<img src="Lecture_1_files/pics/geoid.gif"
     class="gif-restart"
     style="width:100%; max-width:420px; max-height:32vh;">


</div>

</div>


---

## Квазигеоид

<div style="text-align:center; margin: 0.2em 0 0.6em 0;">
  <img src="Lecture_1_files/pics/Quasigeoid.png"
       style="height:35vh; object-fit:contain;">
</div>

- Квазигеоид – геометрическое место точек, получаемых путём откладывания нормальных высот по силовым линиям нормального поля силы тяжести от точек физической поверхности Земли  [Молоденский , 1950;  Серапинас , 2008]
- Возмущающий потенциал силы тяжести

---

## Плоская прямоугольная система координат

**Картографическая проекция** — математически определённое правило перехода от объёмной  
модели Земли к плоскости ≈ **плоская прямоугольная система координат**

$$x = f_1(\varphi, \lambda), \; y = f_2(\varphi, \lambda)$$

Группы проекций по виду вспомогательной фигуры:

- Конические
- Цилиндрические
- Азимутальные


---

## Развёртка на плоскость

:::: {.columns}

::: {.column width="50%"}
![](Lecture_1_files/pics/mandarin_0.jpg){width=84% height=60vh object-fit="contain"}
:::

::: {.column width="50%"}
![](Lecture_1_files/pics/mandarin_1.jpg){width=100% height=60vh object-fit="contain"}
:::

::::

---

## Виды проекций по характеру искажений

:::: {.columns}

::: {.column width="45%"}

Искажаемые параметры:

- Длины  
- Углы  
- Площади  

Виды проекций:

- Равноугольные  
- Равновеликие  
- Произвольные, в т. ч. равнопромежуточные  

:::

::: {.column width="55%"}

![](Lecture_1_files/pics/Area_Distortions.gif)

:::

::::


---

## Индикатриса Тиссо (эллипс искажений)

:::: {.columns}

::: {.column width="55%"}

![](Lecture_1_files/pics/tissot1.png)

![](Lecture_1_files/pics/tissot3.png)

:::

::: {.column width="45%"}

![](Lecture_1_files/pics/tissot2.png)

<a href="https://projections.mgis.psu.edu/"
   target="_blank">
   projections.mgis.psu.edu
</a>

:::

::::

---

## Конические проекции

:::: {.columns}

::: {.column width="50%"}

![](Lecture_1_files/pics/conic_scheme.png)

![](Lecture_1_files/pics/Conic_map.jpg)

:::

::: {.column width="50%"}

<iframe
src="https://www.geogebra.org/classic/nspucyug?embed"
style="width:100%; height:70vh; border:0; overflow:hidden;"
allowfullscreen>
overflow:hidden;</iframe>

:::

::::


---

## Азимутальные проекции

:::: {.columns}

::: {.column width="20%"}

![](Lecture_1_files/pics/az1.png)

![](Lecture_1_files/pics/az2.png)

![](Lecture_1_files/pics/az3.png)

:::

::: {.column width="80%"}

<iframe src="https://www.geogebra.org/classic/ypjb6dgu?embed" width="800" height="600" allowfullscreen style="border: 1px solid #e4e4e4;border-radius: 4px;" frameborder="0"></iframe>

:::

::::

---

## Карты в азимутальных проекциях

:::: {.columns}

::: {.column width="40%"}

![](Lecture_1_files/pics/North_Pole.jpg)

:::

::: {.column width="60%"}

![](Lecture_1_files/pics/Hemispheres.jpg)

:::

::::

---

## Цилиндрические проекции

:::: {.columns}

::: {.column width="55%"}

![](Lecture_1_files/pics/Cylindric.png)


![](Lecture_1_files/pics/Miller.png)

*Проекция Миллера*

:::

::: {.column width="45%"}

![](Lecture_1_files/pics/Mercator.png)

*Проекция Меркатора*

:::

::::


---

## Поперечно-цилиндрические проекции

<iframe src="https://www.geogebra.org/3d/dsnwdbn9?embed" width=100% height=100% allowfullscreen style="border: 1px solid #e4e4e4;border-radius: 4px;" frameborder="0"></iframe>

---

## Проекции Гаусса–Крюгера и UTM

Зональные поперечно-цилиндрические равноугольные проекции

:::: {.columns .polar-columns}

::: {.column width="35%" .polar-img}

![](Lecture_1_files/pics/Zones.png)

*Гаусса–Крюгера*
:::

::: {.column width="65%" .compact-list}

![](Lecture_1_files/pics/UTM.png)

*Universal Transverse Mercator (UTM)*

Счёт зон от 180-го меридиана на восток

:::

::::

**Плоская прямоугольная СК задаётся отдельно для каждой зоны проекции!**

---

## Прямоугольные координаты на топографической карте

:::: {.columns .polar-columns}

::: {.column width="45%" .polar-img}

<p>
<span style="color:#e00000;"><b>6283</b></span> – расстояние в километрах от экватора
</p>

<p>
<span style="color:#1a9d4b;"><b>7</b></span><span style="color:#1f77b4;"><b>409</b></span> – 
<span style="color:#1a9d4b;"><b>номер зоны</b></span> и 
<span style="color:#1f77b4;"><b>расстояние в километрах</b></span> 
от осевого меридиана, сдвинутого на 500 км к западу
</p>

<p>
<span style="color:#f2a900;"><b>6291</b></span>, 
<span style="color:#f2a900;"><b>6776</b></span> – 
то же самое, но для СК соседней зоны
</p>

:::

::: {.column width="55%" .compact-list}

![](Lecture_1_files/pics/Topo.png)

:::

::::

---

## Другие виды проекций

:::: {.columns}

::: {.column width="55%"}

![](Lecture_1_files/pics/Bonne.png){width=100%}
*Проекция Бонна*

:::

::: {.column width="45%"}

![](Lecture_1_files/pics/Fuller.png){width=100%}
*Проекция Фуллера*

![](Lecture_1_files/pics/Good.png){width=100%}
*Проекция Гуда*

:::

::::

---

## Полярные координаты

:::: {.columns .polar-columns}

::: {.column width="45%" .polar-img}

![](Lecture_1_files/pics/Polar.png)

:::

::: {.column width="55%" .compact-list}

- Направление (угол) <span style="color:#e00000;"><b>α</b></span>  
- Расстояние <span style="color:#1f77b4;"><b>L</b></span>

<br>

В качестве направления могут быть взяты:

- Дирекционный угол  
- Истинный азимут  
- Магнитный азимут  
- Условный горизонтальный угол  

<br>

<u>Углы отсчитываются по часовой стрелке</u>

:::

::::

---

## Сближение меридианов

:::: {.columns}

::: {.column width="50%"}

![](Lecture_1_files/pics/Gauss_Proximity.png)

:::

::: {.column width="50%"}

**Гауссово сближение меридианов** — угол между меридианом точки и осевым меридианом проекции

**Магнитное склонение** — угол между истинным и магнитным меридианами

:::

::::

---

##  Сюрприз № 1

![](Lecture_1_files/pics/Surprize_1.png){style="height:100vh; object-fit:contain;"}
